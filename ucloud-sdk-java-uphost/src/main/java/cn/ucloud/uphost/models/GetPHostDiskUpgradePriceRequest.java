...
public class GetPHostDiskUpgradePriceRequest extends Request {
    ...
    /** 磁盘大小，单位GB，必须是10GB的整数倍。系统盘20-500GB，数据盘单块盘20-32000GB。: false */
    @NotEmpty
    @UCloudParam("DiskSpace")
    private Integer diskSpace;

    /** 磁盘 ID。获取扩容价格必填（只能扩不能减）；重装时候不需要填（根据所选盘大小决定）: false */
    @UCloudParam("UDiskId")
    private String uDiskId;

    /** 是否重装价格获取。复用此接口。扩容只能增加云盘大小。重装不限制。枚举值：true/false: false */
