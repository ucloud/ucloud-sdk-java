...
public class GetUEcPodPriceRequest extends Request {
    ...
    /** 支付类型（2按月，3按年，默认2）: false */
    @UCloudParam("ChargeType")
    private Integer chargeType;

    /** 月数或年数（默认值：1，当支付类型为按月时，默认值为0）: false */
    @UCloudParam("ChargeQuantity")
    private Integer chargeQuantity;

    /** 产品类型（normal：标准型，hf：高性能型，默认：normal）: false */
    @UCloudParam("ProductType")
    private String productType;

    /** 是否绑定外网IP（yes：是，no：否，默认：no）: false */
    @UCloudParam("ElasticIp")
    private String elasticIp;

    /** 绑定的带宽，默认0，当绑定外网IP时默认1（单位M）: false */
