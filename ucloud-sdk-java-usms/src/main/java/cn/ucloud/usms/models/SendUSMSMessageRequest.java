...
public class SendUSMSMessageRequest extends Request {
    ...
    /** 短信签名内容，请到[USMS控制台](https://console.ucloud.cn/usms)的签名管理页面查看；使用的短信签名必须是已申请并且通过审核；: false */
    @NotEmpty
     ...
     * 若模板中无可变参数，则该项可不填写；若模板中有可变参数，则该项为必填项，参数个数需与变量个数保持一致，否则无法发送；: false
     */
    @UCloudParam("TemplateParams")
    private List<String> templateParams;

    /** 短信扩展码，格式为阿拉伯数字串，默认不开通，如需开通请联系 UCloud技术支持: false */
    @UCloudParam("ExtendCode")
    private String extendCode;

    /** 自定义的业务标识ID，字符串（ 长度不能超过32 位），不支持 单引号、表情包符号等特殊字符: false */
    @UCloudParam("UserId")
