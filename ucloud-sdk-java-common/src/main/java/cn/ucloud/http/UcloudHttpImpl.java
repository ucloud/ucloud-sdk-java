...
public class UcloudHttpImpl<T> implements UcloudHttp {
    ...
    public Object doGet(BaseRequestParam param, UcloudConfig config, final UcloudHandler handler, Boolean... asyncFlag) throws Exception {
        ...
        final Type resultType = ((ParameterizedType) getClass().getGenericSuperclass()).getActualTypeArguments()[0];
        ...
        if (handler != null) {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        if (responseAsync.getStatusLine().getStatusCode() == 200) {
                            Gson gson = new Gson();
                            BaseResponseResult responseResult = gson.fromJson(content, resultType);
            ...
            try {
                ...
                if (responseSync.getStatusLine().getStatusCode() == 200) {
                    // 正常响应
                    Gson gson = new Gson();
                    responseResult = gson.fromJson(content, resultType);
                } else {
                    // 非200则认为是个异常
