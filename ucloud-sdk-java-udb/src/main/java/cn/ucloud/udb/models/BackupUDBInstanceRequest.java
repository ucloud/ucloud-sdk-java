...
public class BackupUDBInstanceRequest extends Request {
    ...
    /** 是否使用黑名单备份，默认false: false */
    @UCloudParam("UseBlacklist")
    private Boolean useBlacklist;

    /** 使用的备份方式。（快照备份即物理备份。注意只有SSD版本的mysql实例支持设置为snapshot）: false */
    @UCloudParam("BackupMethod")
    private String backupMethod;

    /** 备份黑名单列表，以 ; 分隔。注意：只有逻辑备份下备份黑名单才生效，快照备份备份黑名单下无效: false */
    @UCloudParam("Blacklist")
    private String blacklist;

    /** true表示逻辑备份时是使用 --force 参数，false表示不使用 --force 参数。物理备份此参数无效。: false */
