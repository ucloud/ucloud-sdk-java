...
public class DescribeUDBParamGroupRequest extends Request {
    ...
    /** 参数组id，如果指定则获取描述，否则是列表操作，需要 指定Offset/Limit: false */
    @UCloudParam("GroupId")
    ...
    /** 当请求没有填写Zone时，如果指定为true，表示只拉取跨可用区的相关配置文件，否则，拉取所有机房的配置文件（包括每个单可用区和跨可用区）: false */
    @UCloudParam("RegionFlag")
    private Boolean regionFlag;

    /** 如果未指定GroupId，则可选是否选取特定DB类型的配置(sql, nosql, postgresql, sqlserver): false */
