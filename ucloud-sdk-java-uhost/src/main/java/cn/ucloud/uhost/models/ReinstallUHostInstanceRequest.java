...
public class ReinstallUHostInstanceRequest extends Request {
    ...
    /** UHost实例资源ID 参见 [DescribeUHostInstance](describe_uhost_instance.html): false */
    @NotEmpty
    @UCloudParam("UHostId")
    private String uHostId;

    /**
     * 如果重装UHost实例时LoginMode为Password，则必须填写，如果LoginMode为KeyPair，不需要填写 （密码格式使用BASE64编码；举例如下：# echo -n
     * Password1 | base64UGFzc3dvcmQx。）: false
     */
    @UCloudParam("Password")
    private String password;

    /** 镜像Id，默认使用原镜像 参见 [DescribeImage](describe_image.html): false */
    @UCloudParam("ImageId")
    private String imageId;

    /** 是否保留数据盘，保留：Yes，不报留：No， 默认：Yes；如果是从Windows重装为Linux或反之，则无法保留数据盘（该参数目前仅对本地数据盘起作用）: false */
    @UCloudParam("ReserveDisk")
    private String reserveDisk;

    /** 系统盘大小。 单位：GB， 范围[20,100]， 步长：10: false */
    @UCloudParam("BootDiskSpace")
    private Integer bootDiskSpace;

    /** cloudinit初始化使用。注意：1、总数据量大小不超多16K 2、使用base64编码: false */
    @UCloudParam("UserData")
    private String userData;

    /** 数据盘是否需要自动分区挂载。当镜像支持Cloud-init Feature时可填写此字段。取值“On”（默认值）， “Off”: false */
