...
public class UpdateVServerAttributeRequest extends Request {
    ...
    /** VServer会话保持模式，若无此字段则不做修改。枚举值：None：关闭；ServerInsert：自动生成KEY；UserDefined：用户自定义KEY。: false */
    ...
    /** MonitorType 为 Path 时指定健康检查发送请求时HTTP HEADER 里的域名: false */
    @UCloudParam("Domain")
    private String domain;

    /** MonitorType 为 Path 时指定健康检查发送请求时的路径，默认为 /: false */
    @UCloudParam("Path")
    private String path;
